...
{
  private:
    void _upgrade_recursive(const PointT &p, const std::function<void(const NodeT &, bool)> callback = nullptr);

  protected:
    std::shared_ptr<NodeT> root = nullptr;
  public:
    const pcl::PointXYZ resolution;
    ...
    VoxelOctree(pcl::PointXYZ resolution, pcl::PointXYZ location)
        : resolution(resolution), root(std::shared_ptr<NodeT>(new NodeT(0, resolution, location))) {}
    VoxelOctree(float resolution, pcl::PointXYZ location)
        : resolution(pcl::PointXYZ(resolution, resolution, resolution)),
          root(std::shared_ptr<NodeT>(new NodeT(0, resolution, location))) {}
    void push_pointcloud(const pcl::PointCloud<PointT> &pc, const std::function<void(const NodeT &, bool)> callback = nullptr);
    void push_point(const PointT &p, const std::function<void(const NodeT &, bool)> callback = nullptr);
    /// \brief 获取所有子节点的角点
    pcl::PointCloud<pcl::PointXYZ> get_corner_points();
    /// \brief 遍历所有节点
    void traverse(std::function<void(NodeT &)>);
    void traverse_idx(std::function<void(NodeT &, const std::vector<size_t> &)>);
};

template <typename NodeT, typename PointT>
void VoxelOctree<NodeT, PointT>::push_point(const PointT &p,
                                            const std::function<void(const NodeT &, bool)> callback)
{
    if (!root)
    {
        ...
        root = std::shared_ptr<NodeT>(new NodeT(0, resolution, pcl::PointXYZ(x, y, z)));
    }
    else if (!voxel_op::push_point<NodeT, PointT>(*root, p, callback))
        _upgrade_recursive(p, callback);
}

template <typename NodeT, typename PointT>
void VoxelOctree<NodeT, PointT>::push_pointcloud(const pcl::PointCloud<PointT> &pc,
                                                 const std::function<void(const NodeT &, bool)> callback)
{
    for (auto p : pc)
        push_point(p, callback);
}
...
template <typename NodeT, typename PointT>
void VoxelOctree<NodeT, PointT>::traverse_idx(std::function<void(NodeT &, const std::vector<size_t> &)> func)
{
    if (root)
        voxel_op::recursive_traverse_idx<NodeT>(*root, func);
}
                                                    ...
                                                    const std::function<void(const NodeT &, bool)> callback)
{
    ...
    root = std::shared_ptr<NodeT>(new NodeT(
    ...
    if (!voxel_op::push_point<NodeT, PointT>(*root, point, callback))
        _upgrade_recursive(point, callback);
}
}
